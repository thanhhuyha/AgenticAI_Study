...
PREFERRED_JSON_DIR = "data/dpo_preferred"
# Folder B: rejected responses
REJECTED_JSON_DIR = "data/dpo_rejected"

DATASET_CACHE_DIR = "outputs/dpo_dataset_preferred_vs_rejected"
OUTPUT_DIR = "outputs/qwen30b_dpo_full_fsdp"
...
def build_or_load_dataset() -> Dataset:
    ...
    if cache_dir.exists():
        print(f"Loading cached DPO dataset: {cache_dir}")
    ...
    print(f"Saved DPO dataset with {len(ds)} pairs to {cache_dir}")
    return ds
...
def main() -> None:
    ...
    # Use same model as reference with adapter-free full-param setup.
    # DPOTrainer can run reference-free by passing ref_model=None.
    dpo_args = DPOConfig(
    ...
    print(f"DPO training complete. Saved model to: {OUTPUT_DIR}")
